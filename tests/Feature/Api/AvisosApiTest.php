...
describe('leer avisos, sin token', function () {
    ...
    test('cada aviso trae los campos que decidió el PostResource', function () {
        Post::factory()->create([
            'categoria_id' => $this->categoria->id,
...
describe('escribir avisos', function () {
    test('sin token responde 401', function () {
        $this->postJson('/api/avisos', ['titulo' => 'x'])
            ->assertStatus(401);
    });
    ...
    test('con token crea el aviso y responde 201', function () {
        ...
        Queue::assertPushed(EnviarAvisoPorCorreo::class, fn ($trabajo) => $trabajo->post->titulo === 'Capacitacion el viernes');
    });
